...
export default function GameRound({ gameState, onStartVoting, playSound }: GameRoundProps) {
  ...
  return (
    <div className="flex flex-col h-screen p-4 sm:p-6 bg-background overflow-hidden">
      ...
      <p className="text-sm sm:text-base text-muted-foreground mb-4 sm:mb-8">Iedereen heeft zijn woord gezien. Begin met omschrijven in het echte leven!</p>
      ...
      <div className="flex-1 overflow-y-auto pr-2 mb-4 sm:mb-8">
        <div className="grid grid-cols-2 gap-2 sm:gap-4">
          {gameState.players.map(player => (
            <div 
              key={player.id} 
              className={`p-2 sm:p-4 rounded-xl sm:rounded-3xl border-2 transition-all relative group flex flex-row items-center text-left ${player.isEliminated ? 'bg-secondary border-transparent opacity-50' : 'border-border hover:border-primary/20 bg-card shadow-sm'}`}
            >
              <div className="w-8 h-8 sm:w-12 sm:h-12 shrink-0 bg-secondary rounded-full flex items-center justify-center mr-2 sm:mr-3 text-sm sm:text-2xl">
                👤
              </div>
              ...
              {!player.isEliminated && (
                <button 
                  ...
                  className="absolute top-1 right-1 sm:top-2 sm:right-2 w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-secondary text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-colors shadow-sm"
                >
